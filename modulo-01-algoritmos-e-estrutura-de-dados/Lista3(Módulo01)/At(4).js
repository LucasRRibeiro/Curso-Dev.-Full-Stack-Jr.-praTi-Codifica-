/*
4. Crie um objeto onde cada chave é o nome de um item e o valor é a quantidade
    no inventário do jogador (ex: { espada: 1, poção: 5, escudo: 2 }). Use for...in para
    listar o inventário completo. Permita que o usuário informe um item para usar:
    reduza a quantidade em 1 ou exiba "item esgotado" se for zero.
*/

const tec = require('prompt-sync')()

let inventario = {
    espada: 1,
    pocao: 5,
    escudo: 2,
    arco: 0,
    flecha: 10,
    armadura: 1
}

for (let item in inventario) {
    console.log(`${item}: ${inventario[item]}\n`)
}

let itemUsar = tec("Digite o nome do item que deseja usar: ").toLowerCase()

if (inventario[itemUsar] > 0) {
    inventario[itemUsar] -= 1
    console.log(`\nVocê usou um ${itemUsar}. Quantidade restante: ${inventario[itemUsar]}`)
}

else if (inventario[itemUsar] === 0) {
    console.log(`\nO item ${itemUsar} está esgotado!`)
}

else {
    console.log(`\nO item ${itemUsar} não existe no inventário!`)
}
